...
class PhotoLibraryTest {
	...
	void testErasePhoto() {
		...
		testLibrary.addPhoto(testPhoto1);
		testLibrary.addPhoto(testPhoto2);
		assertTrue("erasePhoto() did not remove a Photograph from PhotoLibrary's ArrayList.", testLibrary.erasePhoto(testPhoto2));
		Album testAlbum = new Album("testAlbumName");
		testLibrary.addPhotoToAlbum(testPhoto2, "testAlbumName");
		assertFalse("erasePhoto() did not remove a Photograph from both PhotoLibrary's ArrayList and Album's ArrayList.", testAlbum.hasPhoto(testPhoto2));
	}
